...
class AzureVisionClient:
    """Azure Computer Vision client for fire detection"""
    ...
    def __init__(self, endpoint: str, key: str):
        ...
        logger.info(
            f"Azure Vision client initialized with endpoint: {endpoint}")
    ...
    def _analyze_fire_detection(self, result):
        ...
        for obj in result.objects:
            if hasattr(obj, 'object_property') and obj.object_property.lower() in fire_keywords:
                ...
                logger.debug(
                    f"Fire object detected: {obj.object_property}, confidence: {obj.confidence}")
        ...
        for tag in result.tags:
            if hasattr(tag, 'name') and tag.name.lower() in fire_keywords:
                ...
                logger.debug(
                    f"Fire tag detected: {tag.name}, confidence: {tag.confidence}")
