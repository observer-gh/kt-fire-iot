...
try {
    $videos = Invoke-RestMethod -Uri "http://localhost:8080/api/cctv/videos" -Method Get
...
try {
    $status = Invoke-RestMethod -Uri "http://localhost:8080/api/cctv/stream/status" -Method Get
...
Write-Host "   http://localhost:8080/cctv-streaming.html" -ForegroundColor Cyan

Write-Host ""
Write-Host "5. WebSocket 연결 테스트:" -ForegroundColor Yellow
Write-Host "   ws://localhost:8080/cctv-websocket" -ForegroundColor Cyan
